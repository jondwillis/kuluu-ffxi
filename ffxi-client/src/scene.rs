//! Scene summarizer — pure `SessionState` → compact natural-language
//! description an LLM can read in ~150 tokens instead of slurping the
//! full 2 kB JSON state on every plan-step.
//!
//! This module is deliberately I/O-free: the renderer is a function of
//! state alone, so it's trivially testable with golden snapshots and so
//! callers (the reactor, the MCP `scene://current` resource) can call
//! it cheaply on demand without coordinating with the session actor.
//!
//! The output is *terse on purpose*. Agents that want richer detail
//! pull `entities://nearby` or `party://members` separately — the scene
//! is the always-on, always-affordable summary.

use serde::{Deserialize, Serialize};

use crate::state::{ChatChannel, EntityKind, SessionState, Stage};

/// Compact view of "what's happening right now". `text` is the
/// human/LLM-readable prose; the structured fields let programmatic
/// consumers (e.g. an MCP client deciding whether to wake the LLM)
/// short-circuit without re-parsing the prose.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct SceneSummary {
    pub text: String,
    /// Stage of the session — present in every summary so the reader
    /// knows whether `text` reflects a live in-zone view or a transient
    /// state (zoning, disconnected).
    pub stage: Stage,
    pub zone_id: Option<u16>,
    /// Self HP %. Sourced from `party` row whose id matches `char_id`
    /// (the `0x0DF GROUP_ATTR` packet self emits cover this).
    pub self_hp_pct: Option<u8>,
    pub self_mp_pct: Option<u8>,
    pub self_tp: Option<u32>,
    pub party_size: u32,
    /// Count of nearby PCs (excluding self).
    pub nearby_pcs: u32,
    pub nearby_npcs: u32,
    pub nearby_mobs: u32,
...
impl SceneSummary {
    /// Render a `SceneSummary` from a `SessionState`. Pure; no I/O.

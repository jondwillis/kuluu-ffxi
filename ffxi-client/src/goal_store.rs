//! Persistent goal store — writes the most-recently-issued goal-setting
//! command to disk so a crash/reboot/disconnect cycle can resume the
//! agent's intent.
//!
//! Persistence intentionally records only the *user-visible command*
//! (Follow / Engage / PathTo / Cancel), not the reactor's internal
//! tactical state (`attack_issued`, in-flight waypoint progress). The
//! supervisor replays the saved command on reconnect; the reactor
//! re-derives tactical state from there.
//!
//! Default location is `$XDG_CONFIG_HOME/ffxi-mcp/goal.json` (falling
...
/// On-disk format. The wrapping struct (rather than serializing
/// `AgentCommand` directly) gives us room to add fields — `set_at_unix`
/// today, maybe a schema version later — without rewriting old files.
...
impl GoalStore {
    ...
    /// `$XDG_CONFIG_HOME/ffxi-mcp/goal.json` or `$HOME/.config/...`.
    /// Errors if neither env var is set (e.g. minimal CI containers) —
    /// callers in those environments should pass an explicit path.
    ...
    /// Load the persisted goal. `Ok(None)` if the file does not exist
    /// (fresh install, or after `clear`); errors only on I/O or parse
    ...
    /// Persist a goal-setting command. Creates parent directories on
    /// demand. Writes are best-effort atomic via tmp-file + rename.
    ...
    /// Remove the persisted goal. `Ok(())` even if the file did not exist.
...
/// Which `AgentCommand` variants represent durable goals worth resuming
/// after a disconnect. `Cancel` is *handled* by clearing the store, so
/// it isn't itself persisted; tactical/transient commands (Move, Chat,
/// Action, Snapshot, etc.) are not durable goals.

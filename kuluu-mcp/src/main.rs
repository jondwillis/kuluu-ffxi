...
async fn read_resource(
    ...
    match uri {
        ...
        "goal://current" => {
            use kuluu_session::state::ReactorGoalSnapshot;
...
fn entities_view(state: &SessionState) -> serde_json::Value {
    ...
    let mut scored: Vec<(&kuluu_session::state::Entity, f32)> = state
        .entities
        .iter()
...
mod tests {
    ...
    fn inventory_events_invalidate_inventory_resource() {
        let upd = AgentEvent::InventoryUpdated {
            container: 0,
            update: kuluu_session::state::InventoryUpdate::SlotChanged {
                slot: kuluu_session::state::ItemSlot {
                    index: 7,
                    item_no: 4112,
                    quantity: 1,
                    locked: false,
    ...
    fn tactical_events_do_not_notify() {
        let cases: Vec<AgentEvent> = vec![
            ...
            AgentEvent::KeyRotated {
                previous_status: kuluu_session::state::BlowfishStatus::Accepted,
            },
            AgentEvent::Error {
                message: "x".into(),
            },
    ...
    fn delivery_commands_report_distinct_kinds() {
        ...
        assert_eq!(
            cmd_kind_label(&AgentCommand::DeliveryBox {
                op: kuluu_session::state::DeliveryBoxOp::Confirm
            }),
            "delivery_box"
        );
    }
    ...
    async fn wait_for_event_returns_matched_payload() {
        ...
        let state = Arc::new(RwLock::new(SessionState::default()));
        let goal_store = GoalStore::new(std::env::temp_dir().join("kuluu-mcp-test-wait-goal.json"));
        let server = FfxiServer::new(cmd_tx, state, goal_store).with_event_tx(event_tx.clone());
    ...
    async fn wait_for_event_times_out_cleanly() {
        ...
        let state = Arc::new(RwLock::new(SessionState::default()));
        let goal_store =
            GoalStore::new(std::env::temp_dir().join("kuluu-mcp-test-wait-timeout-goal.json"));
    ...
    async fn goal_resource_prefers_in_memory_over_stale_disk() {
        ...
        let goal_path = std::env::temp_dir().join(format!(
            "kuluu-mcp-test-goal-prefers-live-{}.json",
            std::process::id()
        ));

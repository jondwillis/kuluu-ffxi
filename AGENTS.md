...
Beads is the **system of record for parity work**, alongside two systems it does not replace:
- `docs/ROADMAP.md` is a directional/broad overview, not a line-by-line tracker. The granular parity backlog lives in the `roadmap`-labelled beads, grounded against the code (each carries `file:line` evidence in its description). Pick work with `bd ready`. The scoreboard's **glyph column is reconciled against beads** by `scripts/roadmap_sync.py` (reads `.beads/issues.jsonl` directly — no `bd` needed): `closed→[x]`, `in_progress→[~]`, `open→[ ]`. `--check` runs in CI (`.github/workflows/roadmap-check.yml`, non-blocking until the known over-claims clear) and flags drift; `--write` only auto-*promotes* a line to `[x]` when its bead is closed — it never downgrades a curated `[x]`/`[~]`, since the `[ ]`-vs-`[~]` distinction is semantic. `scripts/beads-seed-from-roadmap.py` is a **bootstrap-only** importer that seeds naive glyph-derived issues — re-running it overwrites the grounded descriptions/status, so maintain the backlog with `bd` directly instead.
- MEMORY.md auto-memory stays the cross-session memory system — do **not** migrate it into `bd remember`.

GitHub Issues sync is **one-way GitHub → beads** and opt-in: `scripts/beads-github-sync.sh` (manual) or the `workflow_dispatch`-only `.github/workflows/beads-sync.yml`. It upserts GitHub issues into beads keyed on `external_ref: gh-<number>`. The contributor-facing GitHub backlog is published the *other* way by `scripts/create-issues.sh`, which mirrors `docs/ROADMAP.md`'s non-done lines into labelled GitHub issues (idempotent by title). This now runs automatically: `.github/workflows/roadmap-publish.yml` re-publishes on every push to `main` that touches `docs/ROADMAP.md`, so the backlog (and the README's live issue-count badge) stay in sync instead of depending on someone remembering to run the script. These two are **not** a closed loop: `create-issues.sh` publishes the ROADMAP glyph lines, not the grounded `roadmap` beads, so running `beads-github-sync.sh` now would re-import those published issues as fresh `gh-<n>` beads that duplicate the existing roadmap beads (which aren't keyed by gh-ref). A true beads → GitHub push (the grounded descriptions) is still not wired.
...
- **Vanilla parity is the default**; anything with no retail equivalent goes in the Enhanced/addon column of `docs/ROADMAP.md`, gated behind a feature flag.

//! Diagnostic: dump skybox color/altitude keyframes for given zones.
//!
//! Usage: `cargo run --bin dump-skybox -- 101 234`
//!   (101 = East Ronfaure, 234 = Bastok Mines)
//!
//! Reads FFXI_DAT_PATH for the install root, finds the MZB DAT for
//! each requested zone, parses Weather chunks, and prints every
//! keyframe + the interpolated record at V12:00 so we can see whether
//! noon is actually authored as dark or whether something downstream
//! (sRGB conversion, tonemap, shader binding) is breaking it.
...
fn dump_zone(root: &DatRoot, zone_id: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Interpolate the record at V12:00 (noon) the same way
    // skybox.rs::update_skybox does — sample_weather picks the
    // floor record; the renderer then lerps between floor and ceil.
    let noon = 720u32;
...
/// IEC 61966-2-1 sRGB→linear EOTF, same as `bevy::color::Color::srgb().to_linear()`.

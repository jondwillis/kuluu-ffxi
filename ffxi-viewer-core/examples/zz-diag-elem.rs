//! TEMP diagnostic: dump skeleton + skinned-mesh geometry of an NPC by file_id.
...
fn main() {
    ...
    // D3M effect-billboard chunks (the visible flame body).
    ...
    // Img texture names (the pool the renderer resolves D3M tex names against).
...
// (texture-name probe appended below main via a second print is not possible;
// rely on the Img walk done in the renderer.)

//! Render a real FFXI MMB in a Bevy window. End-to-end pipeline:
//!   1. `ffxi-dat::DatRoot` resolves `file_id` → on-disk path
//!   2. Walk chunks, find one of kind 0x2E (MMB)
//!   3. Decrypt with `ffxi-dat::mmb::decrypt`
//!   4. Parse header + sub-records + 40-byte vertex stride + triangle strip
//!   5. Build a `bevy::Mesh` (triangle list) per sub-record
//!   6. Spawn into a scene with an orbit camera + light
//!
//! Usage:
//!   FFXI_DAT_PATH=... cargo run -p ffxi-viewer-core --example mmb-view -- 115 18
...
/// CLI args passed in via env vars so we don't fight Bevy's argv parser.
#[derive(Resource)]
struct LoadRequest {
    file_id: u32,
    chunk_idx: usize,
...
fn setup_scene(mut commands: Commands) {
    // Camera at a distance, looking at origin.
    commands.spawn((
        Camera3d::default(),
    ...
    // Directional light from above-and-side so normals show clearly.
    commands.spawn((
        DirectionalLight {
            illuminance: 9000.0,
            ..default()
...
fn load_and_spawn_mmb(
    ...
    for (i, sub) in sub_records.iter().enumerate() {
        ...
        let mat_handle = materials.add(StandardMaterial {
            ...
            cull_mode: None,
            // Double-sided requires unlit-style normal handling; we let
            // Bevy compute flat normals from positions for safety.
            ..default()
        });

//! Throwaway: dump animation-clip joint coverage + skeleton landmark joints
//! for a PC race, to decide whether `*0`/`*1` clip variants are LOD levels or
//! complementary body-region layers the coordinator composites.
//!
//! Usage: cargo run -p ffxi-viewer-core --example zz-anim-cov -- <skel_id> [motion_id] [clipA clipB ...]
...
fn main() {
    ...
    let motion_id: u32 = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(skel_id + 2600);
    ...
    // Skeleton + landmark joints.
    if let Some(bytes) = read(skel_id) {
        if let Some(skel) = ResourceDir::from_bytes(bytes).collect_skeletons().into_iter().next() {
            ...
            // Parent chain so we can see which joints descend from the feet.
            print!("  parents: ");
    ...
    // Per-clip joint sets across both DATs.
    let mut all = Vec::new();

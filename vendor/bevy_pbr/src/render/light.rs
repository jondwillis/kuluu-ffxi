...
// KULUU PATCH: GPU timing for shadow passes (see vendor/bevy_pbr note in
// workspace Cargo.toml). Upstream shadow passes record no diagnostics span,
// so RenderDiagnosticsPlugin cannot report their GPU cost.
...
fn view_shadow_pass<const IS_LATE: bool>(
    ...
    // KULUU PATCH: record a GPU pass span so RenderDiagnosticsPlugin exposes
    // per-shadow-view GPU time (mirrors main_opaque_pass_3d_node).
    let diagnostics = ctx.diagnostic_recorder();
    let diagnostics = diagnostics.as_deref();

    let depth_stencil_attachment = Some(view_light.depth_attachment.get_attachment(StoreOp::Store));

    // KULUU PATCH: use an encoder-level time_span (not pass_span) — Metal lacks
    // TIMESTAMP_QUERY_INSIDE_PASSES, so timestamps written inside a render pass
    // are silently dropped. Encoder-boundary timestamps (INSIDE_ENCODERS) work.
    // Span name matches the pass label so HUD rows line up with RenderDoc/Metal
    // captures.
    let time_span = diagnostics.time_span(ctx.command_encoder(), view_light.pass_name.clone());
    ...
    // KULUU PATCH: render_pass dropped above; safe to borrow the encoder again.

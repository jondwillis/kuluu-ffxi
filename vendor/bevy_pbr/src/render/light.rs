...
fn view_shadow_pass<const IS_LATE: bool>(
    ...
    // KULUU PATCH: record an encoder-level GPU span for each shadow view.
    // Metal does not expose timestamps inside render passes, while encoder
    // boundary timestamps work with TIMESTAMP_QUERY_INSIDE_ENCODERS.
    ...
    // KULUU PATCH: the render pass must be dropped before borrowing the
    // encoder again to close the diagnostic span.

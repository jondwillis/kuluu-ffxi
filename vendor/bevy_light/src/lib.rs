        ...
        for (view, view_frusta) in &frusta.frusta {
                ...
                .enumerate()
            {
                ...
                for thread_entity_queue in view_visible_entities_queue.iter_mut() {
                    // KULUU PATCH: thread-local queues are resized to this
                    // view's cascade count only inside for_each_init, i.e.
                    // only on threads that received a batch for this view.
                    // A local sized by an earlier par_iter with fewer
                    // cascades survives as a stale, shorter queue here, and
                    // indexing it panics when the cascade count grows at
                    // runtime (e.g. ShadowCascadeCount 2 -> 3 in the
                    // graphics menu). Stale queues were already drained by
                    // the aggregation pass that sized them, so any index
                    // beyond their length holds no entities for this view
                    // and skipping is lossless. Candidate for upstream PR;
                    // bevy main has the same direct index.
                    if let Some(thread_entity_queue) =
                        thread_entity_queue.get_mut(view_dest_index)
                    {
                        view_dest.entities.append(thread_entity_queue);
                    }
                }
                view_dest.shrink();

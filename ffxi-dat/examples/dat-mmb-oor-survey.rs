//! For each `model` sub-record in a DAT, count strip indices that are
//! `>= count` (out of range for the vertex array). These would be
//! filtered out as "bad triangles" by the index-range filter in
//! `dat_mmb.rs`, but if they appear systematically they might be
//! strip-restart sentinels we should be treating specially.
//!
//!   FFXI_DAT_PATH=... cargo run -p ffxi-dat --example dat-mmb-oor-survey -- <file_id>

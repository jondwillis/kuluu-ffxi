//! For a zone DAT, count how often each MMB sub-record's
//! `variant_name` matches an IMG chunk's internal asset name. The
//! match rate predicts how many submeshes get the correct texture
//! vs falling back to the first IMG (which produces the uniform-color
//! "blank" surfaces visible at runtime).
//!
//! Usage:
//!   FFXI_DAT_PATH=... cargo run -p ffxi-dat --example dat-mmb-variant-match -- <file_id>
...
fn main() -> ExitCode {
    ...
    // For each MMB submesh, check whether its variant_name matches.

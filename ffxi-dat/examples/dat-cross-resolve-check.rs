//! For each given zone file_id, parse its MZB placements, classify each
//! against (a) the zone's local MMB asset names (existing resolver),
//! (b) the global MMB index at /tmp/mmb_index.json (built by
//! dat-global-mmb-scan). Print local/cross/miss counts and the
//! distribution of cross-resolved file_ids (library-clustering test).
//!
//! Usage:
//!   FFXI_DAT_PATH=... cargo run --release -p ffxi-dat \
//!     --example dat-cross-resolve-check -- 202 330 200 [/tmp/mmb_index.json]
...
/// Minimal manual JSON loader for the format that dat-global-mmb-scan writes.
/// Returns map: name -> Vec<(file_id, chunk_idx)>.
...
fn main() {
    ...
    for fid in zone_ids {
        ...
        // Apply prefix-and-truncate-to-16 like resolve_mmb_index does, for global lookup too.
        let try_global = |id: &str| -> Option<&Vec<(u32, u32)>> {
            ...
            // Suffix scan over global keys is too expensive; skip.
            None
        };

        // NEW HYPOTHESIS: MMB asset_name is 16 bytes = "<8-char vendor prefix><8-char asset prefix>".
        ...
        for p in &placements {
            ...
            if mzb::resolve_mmb_index(id, &prefix, &mmbs).is_some() {
                ...
                cross += 1;
                // Count distinct file_ids — every (file_id, _) pair contributes
                // to the distribution. Some names span multiple files; counting
                // unique file_ids per match shows which DAT is shared.

//! For every zone_id, resolve the zone's MZB DAT (the same file the viewer
//! loads for collision/weather via `zone_id_to_mzb_file_id`) and scan it for
//! dynamic point-light generators. Confirms whether a zone's point lights
//! live in the DAT the client already loads on zone-in — i.e. whether the
//! light ECS can piggyback on the existing zone-DAT load rather than needing
//! a separate effect-DAT mapping.
//!
//! ```text
//! cargo run --release -p ffxi-dat --example dat-zone-pointlights
//! ```
...
fn main() -> ExitCode {
    ...
    eprintln!(
        "zones with point lights: {zones_with_lights}, total point lights: {total_lights}"
    );
    ExitCode::SUCCESS
}

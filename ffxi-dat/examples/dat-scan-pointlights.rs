//! Scan a DAT file (or a ROM tree recursively) for `Generator` (0x05)
//! chunks whose particle is a dynamic point light (`linkedDataType == 0x47`)
//! and print the parsed [`PointLightDef`] (range, quadratic attenuation,
//! color×2, base offset).
//!
//! This validates [`ffxi_dat::generator::Generator::parse_point_light`]
//! against real retail data — if real braziers/torches surface with sane
//! ranges and warm colors, the lotus-style header model is correct for
//! point-light generators; an empty scan means the parser is reading the
//! wrong offsets (and XIM's section-offset model is needed instead).
//!
//! ```text
//! cargo run --release -p ffxi-dat --example dat-scan-pointlights -- <file_or_dir> [--summary]
//! ```
...
fn main() -> ExitCode {
    ...
    // (generator chunks seen, point-light hits) per file, plus the hits.
    ...
    let mut all_hits: Vec<Hit> = per_file.iter().flat_map(|(_, h)| h.iter().map(clone_hit)).collect();

//! For each MZB placement record (SMZBBlock100), see whether the
//! 16-byte name matches an MMB asset_name in the same zone DAT.
//!
//! Usage:
//!   FFXI_DAT_PATH=... cargo run -p ffxi-dat --example dat-mzb-placement-match -- <file_id>
...
fn main() -> ExitCode {
    ...
    // Build MMB asset_name → chunk_idx map.
    ...
    for i in 0..count {
        ...
        *placements.entry(name.clone()).or_insert(0) += 1;
        // Try: direct / prefix `tshimono` / prefix then truncate to 16.

//! Skeleton mesh section parser (chunk 0x2A) — port of XIM
//! `SkeletonMeshSection.kt`.
//!
//! Faithfully reproduces dual-bone skinning, all five mesh opcodes
//! (textured/untextured strips and meshes), and the symmetry-mirroring
//! that doubles each mesh buffer for `symmetric` models.
//!
//! All `*Offset` header fields are WORD units (multiply by 2 for a byte
//! offset into the chunk body, which == XIM's `dataStartPosition`).
...
pub struct RenderProperties {
    pub t_factor: [u8; 4], // RGBA (XIM nextBGRA -> ByteColor -> GPU order)
...
/// Unpacked joint reference (XIM `JointRef`).
...
/// Source vertex with both joint attachments plus the per-attachment
/// joint refs (kept so the mirror buffer can be rebuilt like XIM).
#[derive(Debug, Clone)]
struct Vertex {
    p0: [f32; 3],
    p1: [f32; 3],
...
/// Final skinned vertex emitted into a [`MeshBuffer`].
#[derive(Debug, Clone)]
pub struct SkinVertex {
    ...
    pub color: [u8; 4], // RGBA (XIM nextBGRA -> ByteColor -> GPU order)
}
...
impl<'a> Cursor<'a> {
    ...
    /// XIM `nextBGRA`: reads stream bytes [b,g,r,a] into ByteColor(r,g,b,a),
    /// which `appendColor` then emits to the GPU as [r,g,b,a]. We return that
    /// same RGBA byte order ([stream2, stream1, stream0, stream3]) so the
    /// produced color matches XIM (and the d3m.rs RGBA convention).
    ...
    /// XIM `nextString` builds the full fixed-width field, keeping the NUL
    /// terminator and any post-NUL padding so the texture lookup key matches
    /// the (identically-built) texture-side name verbatim.
...
/// One emitted (source-vertex, uv, color) record before skinning.
struct MeshVertex {
    src: usize, // index into `vertices`
    u: f32,
    v: f32,
    color: [u8; 4],
}

/// Parse a 0x2A skeleton mesh chunk body.
pub fn parse(id: DatId, data: &[u8]) -> SkelMesh {
    ...
    for _ in 0..single_count {
        ...
        let ref1 = unpack_joint_ref(c.next16()); // usually 0 / ignored
    ...
    // Build a regular + (if symmetric) mirrored MeshBuffer from a record list.
...
/// XIM `flipVertex`: negate p/n along each ref's flip axis and resolve the
/// joint indices from the flipped index.
...
mod tests {
    ...
    /// Header writer helper: 32-byte header. `offsets` are WORD units.
    ...
    /// Build a one-single + one-double vertex mesh with a single tri-mesh of
    /// one triangle (indices 0,0,1). Returns full body bytes.
    fn build_basic_body(f5_symmetric: u8, opcode: u16) -> Vec<u8> {
        // Sections are laid out past the 42-byte header at even byte offsets.
        ...
        let h = HeaderSpec {
            f3: 0,            // no cloth, no jointArray
            f4: 7,            // occludeType
            f5: f5_symmetric, // symmetric flag
        ...
        // counts: 1 single, 1 double
        p16(&mut body, 1);
        p16(&mut body, 1);

        // jointMapping @ mapping_off
        body.resize(mapping_off, 0);
        // single vert: ref0 index=3 flipped=5 axis=1, ref1=0
        let single_ref0: u16 = (1u16 << 14) | (5u16 << 7) | 3u16;
        p16(&mut body, single_ref0);
        p16(&mut body, 0);
        // double vert: ref0 index=2 flipped=6 axis=2, ref1 index=4 flipped=8 axis=3
        ...
        // double vert: p0.x,p1.x,p0.y,p1.y,p0.z,p1.z, w0, w1, n...
        ...
        // instructions @ instr_off
        body.resize(instr_off, 0);
        match opcode {
            0x5453 => {
                // tri strip: numTri=1 -> 3 verts (idx 0,1,0) + uvs
                p16(&mut body, 0x5453);
                p16(&mut body, 1);
                p16(&mut body, 0);
                p16(&mut body, 1);
            ...
            0x0043 => {
                ...
                p16(&mut body, 1);
                p16(&mut body, 0);
                body.extend_from_slice(&[0x11, 0x22, 0x33, 0x44]); // BGRA
            }
            0x4353 => {
                p16(&mut body, 0x4353);
                p16(&mut body, 1); // 1 tri -> 3 verts
                p16(&mut body, 0);
                p16(&mut body, 1);
                p16(&mut body, 0);
                body.extend_from_slice(&[0xAA, 0xBB, 0xCC, 0xDD]); // BGRA
            }
            _ => unreachable!(),
        }
        p16(&mut body, 0xFFFF); // stop
        body
    }

    #[test]
    fn next_bgra_returns_rgba() {
        // Stream order is B,G,R,A; output must be RGBA.
    ...
    fn next_string_keeps_nul_and_padding() {
        // 16-char fixed field with an embedded NUL and post-NUL padding: every
        // byte is preserved as a char, matching XIM's nextString lookup key.
    ...
    fn single_and_double_vertex_decode() {
        ...
        assert_eq!(m.meshes.len(), 1); // not symmetric
        let verts = &m.meshes[0].vertices;
        assert_eq!(verts.len(), 3); // 1 tri = 3 verts

        // First mesh vertex references source vertex 0 (single-jointed).
        assert_eq!(verts[0].p0, [1.0, 2.0, 3.0]);
        assert_eq!(verts[0].joint0_weight, 1.0);
        assert_eq!(verts[0].joint1_weight, 0.0);
        assert_eq!(verts[0].joint_index0, 3); // ref0.index, no joint array

        // Second references source vertex 1 (double-jointed).
    ...
    fn symmetric_doubles_buffers_and_mirrors() {
        ...
        let m = parse(DatId::from_str("0000"), &body);
        assert_eq!(m.meshes.len(), 2); // mirror added

        let normal = &m.meshes[0].vertices;
        let mirror = &m.meshes[1].vertices;

        // Single vert has flip axis 1 (x). Mirror negates p0.x and n0.x,
        // and joint index becomes flippedIndex (5).
        assert_eq!(normal[0].p0, [1.0, 2.0, 3.0]);
        assert_eq!(mirror[0].p0, [-1.0, 2.0, 3.0]);
        assert_eq!(mirror[0].joint_index0, 5);

        // Double vert: ref0 axis 2 (y) for p0/n0, ref1 axis 3 (z) for p1/n1.
        assert_eq!(normal[1].p0, [10.0, 20.0, 30.0]);
        assert_eq!(mirror[1].p0, [10.0, -20.0, 30.0]); // negate y
        assert_eq!(mirror[1].p1, [11.0, 21.0, -31.0]); // negate z
        assert_eq!(mirror[1].joint_index0, 6); // ref0.flippedIndex
        assert_eq!(mirror[1].joint_index1, 8); // ref1.flippedIndex
                                               // weights/uv unchanged
    ...
    fn tri_strip_count() {
        ...
        let m = parse(DatId::from_str("0000"), &body);
        // numTri=1 -> numVertices = numTri + 2 = 3
    ...
    fn untextured_tri_mesh_carries_bgra() {
        ...
        assert_eq!(m.meshes[0].mesh_type, MeshType::Mesh);
        assert_eq!(m.meshes[0].vertices.len(), 3);
        // Stream bytes [0x11,0x22,0x33,0x44] (B,G,R,A) -> RGBA [0x33,0x22,0x11,0x44].
    ...
    fn single_color_untextured_tri_strip_carries_bgra() {
        ...
        let m = parse(DatId::from_str("0000"), &body);
        assert_eq!(m.meshes[0].mesh_type, MeshType::Strip);
        assert_eq!(m.meshes[0].vertices.len(), 3); // numTri=1 -> 3 verts
                                                   // Stream bytes [0xAA,0xBB,0xCC,0xDD] (B,G,R,A) -> RGBA [0xCC,0xBB,0xAA,0xDD].

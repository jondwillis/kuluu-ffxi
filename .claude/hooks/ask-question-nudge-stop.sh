...
reason='Stop-hook checkpoint: this turn ends by asking the user something in prose. If you are genuinely waiting on a decision or choice, re-pose it with the AskUserQuestion tool — gather the question(s) and offer concrete, mutually-exclusive options (mark a recommended one first) so the user can pick rather than type. Use the elicitation tool when you need free-form input instead. If the trailing "?" was rhetorical, an aside, or already answered elsewhere, do nothing and stop — this checkpoint stays quiet until the question text changes.'

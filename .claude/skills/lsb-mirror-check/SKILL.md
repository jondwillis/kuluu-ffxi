...
- `~/.claude/projects/-Volumes-Sidecar-ffxi/memory/server_is_lsb_not_phoenix.md`
  — why LSB is authoritative
- `ffxi-proto/build.rs`, `ffxi-nav/build.rs` — existing scraper
  patterns to extend for new numeric-constant tables (see the
  `vendor-scrape` skill)
- `.claude/agents/protocol-conformance-reviewer.md` — the subagent
  that audits a *diff* against vendor sources; use that for code
  review, this skill for "I'm about to write X"

...
fn main() -> Result<()> {
    ...
    // The coordinates are intentionally rounded (`{:.3}`/`{:.6}`) data values;
    // silence the pedantic literal lints on the generated table rather than
    // mangling the emitted numbers. `approx_constant` fires on rotations that
    // happen to equal pi / pi-2 — they're zone data, not the math constant.
    out.push_str("#[allow(clippy::excessive_precision, clippy::approx_constant)]\n");

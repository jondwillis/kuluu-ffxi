...
use kuluu_render::dat_mzb::{MzbCollisionGeometry, FLOOR_NORMAL_MIN};
...
pub fn process_debug_heights(
    ...
    mut toasts: MessageWriter<kuluu_render::snapshot::ToastEvent>,
) {
    if events.is_empty() {
        return;
    }
    for _ in events.read() {
        ...
        let nearest_mob = scene_state
            ...
            .entities
            .iter()
            .filter(|e| matches!(e.kind, kuluu_snapshot::EntityKind::Mob))
            .min_by(|a, b| {
        ...
        match nearest_mob {
            ...
            Some(mob) => {
                ...
                let mob_bevy = kuluu_render::ffxi_to_bevy(mob.pos);
...
fn push(toasts: &mut MessageWriter<kuluu_render::snapshot::ToastEvent>, text: String) {
    toasts.write(kuluu_render::snapshot::ToastEvent::debug(text));
}

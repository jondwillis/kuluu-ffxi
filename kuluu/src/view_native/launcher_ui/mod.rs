...
fn sync_window_title(
    ...
    let new_title = format!("kuluu - {}", server.display_label());
...
#[derive(Resource, Default)]
pub(crate) struct CharListData(pub Vec<kuluu_session::lobby_client::CharSlot>);

#[derive(Resource, Default)]
pub(crate) struct SelectedChar(pub Option<kuluu_session::lobby_client::CharSlot>);
...
fn populate_self_appearance(
    ...
    mut appearance: ResMut<kuluu_render::scene::SelfAppearance>,
) {
    ...
    app.add_systems(
        ...
        (
            ...
            char_preview::relight_preview_actor
                .after(kuluu_render::ffxi_actor_render::update_ffxi_render_actor_lighting),
        )
            .chain()
            .run_if(in_state(LauncherState::CharList)),
    );

//! Packet integrity check used by the map server.
//!
//! `server/src/common/utils.cpp::checksum()` is just MD5(buf) == expected; this
//! module exposes a clearer-named `verify_md5` plus the trailing-hash convention
//! used in the FFXI map packet wire format.

use crate::md5::md5;

/// Returns true iff `MD5(payload) == expected`.
pub fn verify_md5(payload: &[u8], expected: &[u8; 16]) -> bool {
    &md5(payload) == expected
}

/// Compute MD5 over `payload` and write it into the trailing 16 bytes of
/// `frame`. Caller guarantees `frame.len() >= 16` and that `payload` is the
/// portion of the frame the trailer should authenticate (typically
/// `&frame[FFXI_HEADER_SIZE .. frame.len() - 16]`).

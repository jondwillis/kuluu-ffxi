//! Zone-id → zone-name lookup for Detour navmesh files.
//!
//! LSB ships its `.nav` files name-keyed (e.g. `West_Sarutabaruta.nav`,
//! `Rabao.nav`) with only a handful of legacy numeric exceptions
//! (`133.nav`, `229.nav`, `49.nav`). Without this lookup the loader
//! can only find the numeric few; with it, the navmesh-aware path can
//! resolve any of the ~300 zones in the SQL table.
//!
//! Source: extracted from `server/sql/zone_settings.sql` (LSB
//! upstream). Re-extract whenever LSB adds a new zone:
//!
//! ```text
//! grep "INSERT INTO" server/sql/zone_settings.sql | \
//!   sed -E "s/.*VALUES \(([0-9]+),[0-9]+,'[^']*',[0-9]+,'([^']+)',.*/    (\1, \"\2\"),/"
//! ```

/// Resolve a zone id to its canonical SQL name (with underscores for
/// spaces). Returns `None` for ids outside the LSB table.
pub fn zone_name(zone_id: u16) -> Option<&'static str> {
    ZONE_TABLE
        .iter()
        .copied()
        .find_map(|(id, name)| (id == zone_id).then_some(name))
}

/// Dense table of all known zones from `zone_settings.sql`. Index is
/// not zone_id (some zones are placeholder / unused), so callers go
/// through `zone_name`.
const ZONE_TABLE: &[(u16, &str)] = &[
    (0, "unknown"),
    (1, "Phanauet_Channel"),
...
mod tests {
    ...
    #[test]
    fn known_zones_round_trip() {
        // Spot-check a handful: the legacy numeric files (133, 229, 49)
        // and a name-keyed zone we actively use for farming.
        assert_eq!(zone_name(115), Some("West_Sarutabaruta"));
        assert_eq!(zone_name(247), Some("Rabao"));
        assert_eq!(zone_name(249), Some("Mhaura"));
        assert_eq!(zone_name(133), Some("Outer_RaKaznar_[U2]"));
    }

    #[test]
    fn unknown_zone_returns_none() {
        // 65535 is a sentinel value never assigned in zone_settings.sql.
        assert_eq!(zone_name(65535), None);
    }
}
